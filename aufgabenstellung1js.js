...
$(window).on('load', function() {
 mData=",loadedPage,Aufgabenstellung1,"+Date.now()+",,,,,";
    fetchFunction(mData)})


$(window).focus(function() {
    mData=  ",switchedBack,Aufgabenstellung1,"+Date.now()+",,,,,";
    fetchFunction(mData)
});

$(window).blur(function() {
mData=  ",switchedWindow,Aufgabenstellung1,"+Date.now()+",,,,,";
    fetchFunction(mData)});
...
function checkTabPress(e) {
    ...
    if (e.keyCode === 9 ) {
        mData= ",pressedTab,"+e.target.id+","+ Date.now() +",,,,,"
        fetchFunction(mData)
    }
}
function checkTabUp(e) {
    'use strict';
    var ele = document.activeElement;

    if (e.keyCode === 9 ) {
        mData= ",releasedTab,"+e.target.id+","+ Date.now() +",,,,,"
        fetchFunction(mData)
    }
}
...
function fetchFunction(mData){
    fetch(`${window.origin}/Aufgabenstellung1`, {
            method: "POST",
            credentials: "include",
