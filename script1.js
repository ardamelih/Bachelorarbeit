...
formElements.push(document.getElementById("titleAnzeige"))
formElements.push(document.getElementById("zoomIn"))
formElements.push(document.getElementById("zoom"))
...
formElements.push(document.getElementById("leafletBottomRight"))
...
$(window).on('load', function() {
 mData=",loadedPage,Aufgabe1,"+Date.now()+",,,,,";
    fetchFunction(mData)})

//get details when the user changed back from another tab or window
$(window).focus(function() {
    mData=  ",switchedBack,Aufgabe1,"+Date.now()+",,,,,";
    fetchFunction(mData)
});

//get details when the user changed to another tab or window
$(window).blur(function() {
mData=  ",switchedWindow,Aufgabe1,"+Date.now()+",,,,,";
    fetchFunction(mData)});


function scrollFunction() {
     mData =",scrolled,,"+  Date.now() + ","    +     event.clientX.toString() + "_" + event.clientY.toString()+",,,,"
    fetchFunction(mData)
    }
...
//save the details, when a user has clicked on an html element
function focusFunction(event) {
  if(event.target.className==="geocoder-control-input leaflet-bar"){
    ...
    if(event.target.id==="mySlider"){
            ...
            var sliderRange=parseInt(event.target.value)+1
